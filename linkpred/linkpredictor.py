...
transform = T.Compose([
    ...
    T.ToDevice(device),
    T.RandomLinkSplit(num_val = 0.05, num_test = 0.01, is_undirected = True, add_negative_train_samples = False),
])
...
def train(data):
    ...
    neg_edge_index = negative_sampling(edge_index = data.edge_index, num_nodes = data.num_nodes, num_neg_samples = None, method = 'sparse')
    edge_label_index = torch.cat([data.edge_index, neg_edge_index], dim = -1, )
    edge_label = torch.cat([torch.ones(data.edge_index.size(1)), torch.zeros(neg_edge_index.size(1))], dim = 0).to(device)
    out = model.decode(z, edge_label_index).view(-1).to(device)
...
def test(loader):
    ...
    out = model.decode(z, data.edge_index).view(-1).sigmoid().to(device)
    pred = (out > threshold).float() * 1
    score = f1_score(np.ones(data.edge_index.size(1)), pred.cpu().numpy())
    scores.append(score)
...
def predictions(run_id, max = 1000, threshold = 0.99):
    ...
    neg_edge_index = negative_sampling(edge_index = data.edge_index, num_nodes = None, num_neg_samples = None, method = 'sparse')
    out = model.decode(z, neg_edge_index).view(-1).sigmoid().to(device)
